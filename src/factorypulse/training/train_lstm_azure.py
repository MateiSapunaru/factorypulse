...
def load_lstm_config(config_path: str | Path) -> LSTMSection:
    ...
    allowed_keys = {field.name for field in fields(LSTMSection)}
    filtered_lstm = {key: value for key, value in raw_lstm.items() if key in allowed_keys}

    return LSTMSection(**filtered_lstm)
...
def train_model(
    ...
    for epoch in range(num_epochs):
        ...
        mlflow.log_metric("epoch_train_loss", mean_loss, step=epoch + 1)
...
def main() -> None:
    args = parse_args()
    config = load_lstm_config(args.config_path)
    ...
    train_df = pd.read_csv(data_dir / "train_features.csv", parse_dates=["timestamp"])
    val_df = pd.read_csv(data_dir / "val_features.csv", parse_dates=["timestamp"])
    test_df = pd.read_csv(data_dir / "test_features.csv", parse_dates=["timestamp"])
    ...
    mlflow.log_param("model_name", "lstm_autoencoder")
    log_params_flat(
        {
    ...
    log_params_flat(config.__dict__, prefix="lstm_autoencoder")
    ...
    threshold, best_val_f1_from_search, threshold_df = search_best_threshold_with_fp_constraint(
        ...
        scores=val_scores,
        num_thresholds=config.threshold_search_points,
        max_fp_per_1000=config.max_fp_per_1000,
    )
    ...
    log_text_artifact(
        content=(
            "Azure ML LSTM autoencoder job. "
            "Model trained on normal train sequences only. "
            "Final threshold selected by maximizing validation F1 under a "
            f"false-positive constraint of {config.max_fp_per_1000:.1f} FP/1000. "
            "Deployable bundle includes model weights, scaler, and metadata."
        ),
        artifact_file="notes/lstm_autoencoder_azure_summary.txt",
    )
